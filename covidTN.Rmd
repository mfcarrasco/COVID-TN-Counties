...
tn_daily$County = gsub(' County', '', tn_daily$County)
tn_daily$County = ifelse(tn_daily$County == grep('Out', tn_daily$County,ignore.case=TRUE, value= TRUE),
                         "Out of TN",
                         tn_daily$County)

tn_daily$County = ifelse(tn_daily$County == grep('total', tn_daily$County,ignore.case=TRUE, value= TRUE),
                         "Total",
                         tn_daily$County)
...
tn_daily2 = tn_daily2[!(tn_daily2$county=='Total' | tn_daily2$county =='Out of TN' | tn_daily2$county =='Pending'),]
...
```{r}
tn_daily$Case_Per = round((tn_daily$Positive/(tn_daily$Positive+tn_daily$Negative+tn_daily$Death))*100,1) 
tn_daily$Negative_Per = round((tn_daily$Negative/(tn_daily$Positive+tn_daily$Negative+tn_daily$Death))*100,1) 
tn_daily$Deaths_Per = round((tn_daily$Death/(tn_daily$Positive+tn_daily$Negative+tn_daily$Death))*100,1) 

total_cases = tn_daily[which(tn_daily$County =='Total'),'Positive'] %>% formattable::comma(digits=0)
per_total_cases = tn_daily[which(tn_daily$County =='Total'),'Case_Per'] %>% paste0('%')
total_cases = paste0(total_cases, ' (',per_total_cases,')')  
...
per_total_neg = tn_daily[which(tn_daily$County =='Total'),'Negative_Per'] %>% paste0('%')
total_negatives = paste0(total_negatives, ' (', per_total_neg, ')')
...
per_total_death = tn_daily[which(tn_daily$County =='Total'),'Deaths_Per'] %>% paste0('%')
total_death = paste0(total_death, ' (', per_total_death, ')')
...
#Data Cleaning
tn_age = tn_age[[1]]
names(tn_age) = c('Age_Ranges', 'Count', 'Deaths')
tn_age = tn_age[which(
  tn_age$Age_Ranges != grep('p', tn_age$Age_Ranges,ignore.case=TRUE, value= TRUE)),]
...
ggplot(data=tn_age, aes(x=Age_Ranges))+
  ...
  geom_text(aes(y=Count,label =paste0(Percent, '%' )), vjust=-0.2)+
  geom_text(aes(y=Deaths,label =paste(Deaths)), vjust=-0.2, color='white')+
