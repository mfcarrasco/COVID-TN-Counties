...
### Positive cases by counties with more than 50 cases {.no-mobile}
...
tn_delta =  readxl::read_excel('TN_COVID19_COUNTYDaily.xlsx',sheet=1) %>%
  ...
  mutate(RATE_CHG_1DAY = RATE_CHG_1DAY*100) %>%
  filter(DATE >= (Sys.Date()-28))%>%
  filter(COUNTY != 'Balance', COUNTY != 'Probable') %>%
...
ggplot(total_delta, aes(x=DATE)) + 
  ...
  geom_line(aes(y=ma(RATE_CHG_1DAY), color = 'Moving Average' ),size = 1,linetype='dashed')+
...
ggplot(total_delta, aes(x=DATE)) + 
  ...
  geom_line(aes(y=ma((CASES_NEW)) , color = 'Moving Average' ),size = 1,linetype='dashed')+
  labs(x='', y='New Cases')+
  theme(legend.title = element_blank(), 
        panel.background = element_blank(), 
...
<!-- colors = c('Shelby County' = '#002D65', 'Moving Average' = '#CC0000') -->
<!-- ggplot(tn_delta[which(tn_delta$COUNTY=='Shelby'),], aes(x=DATE)) +  -->
<!--   geom_col(aes(y=(CASES_NEW), fill= 'Shelby County'),size=.5)+ -->
<!--   geom_line(aes(y=ma((CASES_NEW)) , color = 'Moving Average' ),size = 1,linetype='dashed')+ -->
<!--   labs(x='', y='New Cases')+ -->
<!--   theme(legend.title = element_blank(),  -->
<!--         panel.background = element_blank(),  -->
...
<!-- ### New Cases in Davidson County -->
<!-- ```{r} -->
<!-- #Moving Average for Davidson County -->
<!-- colors = c('Davidson County' = '#002D65', 'Moving Average' = '#CC0000') -->
<!-- ggplot(tn_delta[which(tn_delta$COUNTY=='Davidson'),], aes(x=DATE)) +  -->
<!--   geom_col(aes(y=(CASES_NEW), fill= 'Davidson County'),size=.5)+ -->
<!--   geom_line(aes(y=ma((CASES_NEW)) , color = 'Moving Average' ),size = 1,linetype='dashed')+ -->
<!--   labs(x='', y='New Cases')+ -->
<!--   theme(legend.title = element_blank(),  -->
<!--         panel.background = element_blank(),  -->
