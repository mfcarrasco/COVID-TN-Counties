...
tn_ext$COUNTY = sub('ﬀ', "ff", tn_ext$COUNTY) #Coffee and Jefferson have some weird issues.
...
total_death_vb = total_death %>% formattable::comma(digits=0) %>% paste0(' (',death_per,')')  
valueBox(value = total_death_vb, color='#002D65')
```
...
<!-- ```{r}  -->
<!-- #Recovered -->
<!-- recov_per = ((total_recov/total_cases)*100) %>% round(1) %>% paste0('%') -->
<!-- total_recov_vb = total_recov %>% formattable::comma(digits=0) %>% paste0(' (',recov_per,')')  -->
<!-- valueBox(value = total_recov_vb, color='#002D65') -->
<!-- ``` -->
...
<!-- ```{r}  -->
<!-- #Active Cases -->
<!-- active_per = ((active_cases/(total_cases-total_death))*100) %>% round(1) %>% paste0('%') -->
<!-- active_cases_vb = active_cases %>% formattable::comma(digits=0) %>% paste0(' (',active_per,')')   -->
<!-- valueBox(value = active_cases_vb, color='#002D65') -->
<!-- ``` -->

### Active Cases
```{r}
active_per = ((active_cases/(total_cases-total_death))*100) %>% round(1) 
